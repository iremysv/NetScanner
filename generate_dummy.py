...
for i in range(60):
    packets.append(
        ...
        / UDP(sport=4321, dport=53)
        / DNS(rd=1, qd=DNSQR(qname="malicious-tunnel.com"))
    )
...
for i in range(110):
    packets.append(
        ...
        / TCP(sport=5555, dport=80, flags="S")
    )

# 4. HTTP DPI Anomaly (SQL Injection)
packets.append(
    IP(src="172.16.0.5", dst="192.168.1.200")
    / TCP(sport=3333, dport=80, flags="PA")
    / Raw(
        load=(
...
packets.append(
    ...
    / TCP(sport=4444, dport=443, flags="PA")
    / Raw(load=large_data)
)

# 6. GeoIP Test
packets.append(
    IP(src="104.21.43.12", dst="192.168.1.100")
